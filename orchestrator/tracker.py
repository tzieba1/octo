...
class IssueTracker:
    ...
    def _load_provider(self, name: str) -> ProviderInterface:
        ...
        if name == 'github':
            ...
            from orchestrator.providers.gitlab import GitLabProvider
            return GitLabProvider()
        elif name == 'gitea':
            from orchestrator.providers.gitea import GiteaProvider
...
---
*This issue is part of a cross-repository epic managed by repo-orchestrator*
            """
    ...
    def link_related_issues(self, issue_links: List[tuple[str, int, str, int]]) -> List[bool]:
        ...
        for repo1, issue1, repo2, issue2 in issue_links:
            try:
                success = self.provider.link_issues(repo1, issue1, repo2, issue2)
                results.append(success)
                
                if success:
                    logger.info(f"Linked {repo1}#{issue1} <-> {repo2}#{issue2}")
            except Exception as e:
...
---
*Generated by repo-orchestrator*
            """
            ...
            try:
                ...
                logger.error(f"Failed to create release checklist in {repo}: {e}")
            ...
            try:
                ...
                for issue in all_issues:
                    for label in issue.get('labels', []):
                        ...
                        repo_metrics['labels'][label_name] = repo_metrics['labels'].get(label_name, 0) + 1
                        metrics['by_label'][label_name] = metrics['by_label'].get(label_name, 0) + 1
        ...
        for repo in repos:
            for label in labels:
                try:
                    ...
                    logger.error(f"Failed to create label {label['name']} in {repo}: {e}")
